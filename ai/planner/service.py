...
class LLMIntentPlanner(IntentPlanner):
    ...
    def __init__(self, gateway: LanguageModelGateway, prompt_manager: PromptManager | None = None) -> None:
    ...
    async def create_plan(self, request: ChatRequest) -> ExecutionPlan:
        ...
        try:
            ...
            logger.warning("LLM returned non-JSON plan, falling back to direct_chat: %s", e)
            return ExecutionPlan(
                intent="direct_chat",
                confidence=0.5,
                steps=[]
            ...
            logger.error("Unexpected error during plan creation, falling back to direct_chat: %s", e, exc_info=True)
            return ExecutionPlan(
                intent="direct_chat",
                confidence=0.5,
                steps=[]
